...
package injector.injection.impl;
...
public class Injector {
    ...
    private ConcurrentHashMap<Class<?>, List<NameBind>> bindings;
    private ConcurrentHashMap<Class<?>, Object> instances;
    private Logger logger;

    private Injector() {
        this.bindings = new ConcurrentHashMap<>();
        this.instances = new ConcurrentHashMap<>();
        this.logger = new ConsoleLogger(this.getClass().getName(), ConsoleLogger.DEFAULT_FORMAT,
    ...
    public static boolean selfinjectLogger() {
        ...
        if (factory == null) {
            Injector.instance.logger.logDebug("Self-injecting logger failed");
            return false;
        }
        Injector.instance.logger = factory.getLogger(Injector.class);
        Injector.instance.logger.logDebug("Self-injecting logger succeded.");
        return true;
    }
     ...
     * Default constructor without any arguments is going to be used for creating instance of
     * given class.
     * @param <T> interface that has already been binded
     * @param clazz Class object representing interface that you wish to gain instance of.
     * @return true if successful, false otherwise
     */
    public static <T> T inject(Class<T> clazz) {
        return Injector.instance.getInstance(clazz, Injector.DEFAULT_NAME, new Object[0]);
    }
     ...
     * implementation. Default constructor without any arguments is going to be used for creating
    ...
    public static <T> T inject(Class<T> clazz, String named) {
        return Injector.instance.getInstance(clazz, named, new Object[0]);
    }
     ...
     * @param clazz Class object representing interface that you wish to gain instance of.
     * @param named name of the implementation you wish to use (name provided by @Named annotation)
     * @param args are optional arguments that are supposed to be passed to constructor.
     * @return true if successful, false otherwise
     */
    public static <T> T inject(Class<T> clazz, String named, Object ... args) {
        return Injector.instance.getInstance(clazz, named, args);
    }
    ...
    private <T> T getInstance(Class<T> inf, String named, Object[] args) {
        boolean knownClass;
        T inst = null;
        ...
        if (!knownClass) {
            ...
            inst = this.createInstance(inf, args);
        ...
        if (knownClass) {
            ...
            for (NameBind bind : bindList) {
                ...
                newInstance = this.createInstance(dependenci, args);
    ...
    private <T> T createInstance(Class<T> inf, Object[] arguments) {
        if (this.instances.containsKey(inf)) {
            ...
            // this should never happend beacuse binding checks if assigneable
            catch(ClassCastException ex) {
                logger.logError("Singleton class '" + inf.getName() + "' has incompatible binding.", ex);
                return null;
            }
        }
        ...
        try {
            ...
            Constructor[] constructors = inf.getConstructors();
            ...
            for (Constructor c:constructors) {
                if (c.getParameterCount() == arguments.length) {
                    ...
                    for (int i = 0; i < arguments.length && i < parameters.length; i++) {
                        if (arguments[i].getClass() != parameters[i]) {
                            fits = false;
                            break;
                        }
                    }
                    ...
                    logger.logDebug("Found desired constructor for '" + inf.getName() + "' class.");
                    selected = c;
                    break;
                }
            }
            ...
            if (selected == null) {
                selected = inf.getConstructor();
            }

            infInstance = selected.newInstance(arguments);
            ...
            for (Field f : fields) {
                ...
                logger.logDebug("Trying to get instance for '" + clazz.getName() + "' and '"
                        + f.getName() + "' field.");
                f.set(infInstance, this.getInstance(clazz, annotation.named(), new Object[0]));
            }
